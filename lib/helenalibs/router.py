...
class Routers:
    def __init__(self, hostname, ip, username, password, secret, ios_os, out, log_path):
        ...
        self.errorlog = self.log_path[0:3]+"/error/" #folder
        self.errorlog_path = self.errorlog+self.log_path[4:-4]+"-error.log" #full-path
    ...
    def connect(self, command, i):
        ...
        while retry < CONNECT_RETRY:
            ...
            try:
                ...
                retry +=1
                err = (f"{self.hostname} : Failed to connect ({retry})")
                logging.error(err)
                self.logging_error(err, e)
        ...
        try:
            with open(f"{self.raw_path}{self.hostname}_{self.command}_{TIMESTAMP}.csv", mode="w", newline="") as csv_file:
                ...
                for row in self.parsed_output:
                    writer.writerow(row)
            logging.info(f"{self.hostname} : Output saved to {self.raw_path}{self.hostname}_{self.command}_{TIMESTAMP}.csv")
        except Exception as e:
            err = (f"{self.hostname} : Failed saving to {self.raw_path}{self.hostname}_{self.command}_{TIMESTAMP}.csv")
            logging.error(err)
            self.logging_error(err, e)
    ...
    def parse_cpu_history(self):
        try:
            with open(f"{self.cpu_history_path}{self.hostname}_{self.command}_{TIMESTAMP}.txt", mode="w", newline="") as txtfile:
                ...
                logging.info(f"{self.hostname} : Output saved to {self.cpu_history_path}{self.hostname}_{self.command}_{TIMESTAMP}.txt")
        except Exception as e:
            err = (f"{self.hostname} : Failed saving to {self.cpu_history_path}{self.hostname}_{self.command}_{TIMESTAMP}.txt")
            logging.error(err)
            self.logging_error(err, e)
        ...
        if self.connect("custom", 0):
            for c in custom_commands:
                ...
                if "Invalid" in self.output:
                    err = (f"{self.hostname} : Invalid command '{self.command}'")
                    logging.error(err)
                    self.logging_error(err)
    ...
    def export_custom_command(self):
        try:
            with open(f"{self.date_path}{self.hostname}_custom_{TIMESTAMP}.txt", mode="a") as txtfile:
                txtfile.write('\n---------------------------------------------------------------------------\n')
                txtfile.write(self.command)
                txtfile.write('\n---------------------------------------------------------------------------\n')
                txtfile.write(self.output)
                txtfile.write('\n---------------------------------------------------------------------------\n\n\n')
                logging.info(f"{self.hostname} : Output appended to {self.date_path}{self.hostname}_custom_{TIMESTAMP}.txt")
        except Exception as e:
            err = (f"{self.hostname} : Failed appending to {self.date_path}{self.hostname}_custom_{TIMESTAMP}.txt")
            logging.error(err)
            self.logging_error(err, e)